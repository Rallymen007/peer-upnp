...
var Device = function(peer, options){
	...
	this.descriptionURL = this.peer.prefix+"/" + this.uuid + "/";
...
var Service = function(device, options){
	...
	this.SCPDURL = this.device.peer.prefix + "/" + this.USN + "/service/desc.xml";
	this.controlURL = this.device.peer.prefix + "/" + this.USN + "/service/control";
	this.eventSubURL = this.device.peer.prefix + "/" + this.USN + "/service/events";
	this.configId = 1;
...
Service.prototype.notify = function(){
	...
	for ( var sid in this.subscriptions) {
		...
		if(!subscription || !subscription.callbacks) continue;
...
var registerHTTPHandler = function(peer){
	peer.server && peer.server.on("request", peer.httpHandler = function(req,rsp){
		...
		if (isPeer) {
			var fullpath = url.pathname.substr(peer.prefix.length);
			let splitindex = fullpath.indexOf("/",1);
			
			let path = fullpath.slice(splitindex);
			let udn = fullpath.slice(1, splitindex);
			let usn = path.slice(1, path.indexOf("/",1));
			path = path.slice(path.indexOf("/",1));
			
			// upnp device spy doesnt technically follow the spec lol
			// since it would remove the udn from the path because it doesnt know
			// what relative means, check if usn if available starts with udn
			// if not prepend udn to path and set usn to udn
			if(path !== "/" &&  (!usn || !usn.startsWith(udn))){
				path = "/" + usn + path;
				usn = udn;
			}
			
			// easiest way to reduce code change
			url.query=querystring.parse("udn=" + udn + "&usn=" + usn);
			...
			//if(!handler) console.log(handler);
...
var handlePostControl = function(req,rsp,peer){
	...
	if (split.length==2) {
		...
		if (service && actionName) {
			xml2js.parseString(req.data,{mergeAttrs: true, explicitArray: false, ignoreXmlns: true, ignoreAttrs: true},async function(err,json){
				if (err) {
					rsp.statusCode = 400;
					...
					try {
						...
						try {
							let wret = service.implementation[actionName].call(service,inputs);
							if(wret instanceof Promise){
								wret = await wret;
							} 
							options.outputs = wret || {};
						} catch (e) {
...
var httpHandlers = {
	"GET /": handleGetDeviceDescription,
